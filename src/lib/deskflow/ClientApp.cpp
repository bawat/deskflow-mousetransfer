...
// backoff bought was slower recovery. It is the PRIMARY speedup for a game-mode resume, where every
// machine's core restarts at once and a client that dials a server still coming up must keep
// re-dialing until it is listening — the mouse cannot cross to that client until then. The connect
// timeout (Client::setupTimer) is kept generous (3.0s) for slow-hardware handshakes, so this 0.25s
// backoff is what actually paces the re-dials once an attempt is exhausted. (Retrying an off peer
// every ~3.25s is trivial traffic — one packet to one known server, not a broadcast sweep.)
